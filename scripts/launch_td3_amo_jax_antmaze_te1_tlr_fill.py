#!/usr/bin/env python3
"""JAX handoff for remaining amo_log main td3_amo antmaze alpha_E=alpha_B=2 T_lr cells.

Reads pending cells from:
  /home/choi/AMO_store/td3_amo_jax_antmaze_te1_tlr_fill_handoff/pending_cells.json
Uses /home/choi/amo train.py --backend jax (release protocol: train-only +
save-every 20k; CPU eval is a separate reaper).
"""
...
OUT = Path("/home/choi/AMO_store/td3_amo_jax_antmaze_te1_tlr_fill")
HANDOFF = Path(
    "/home/choi/AMO_store/td3_amo_jax_antmaze_te1_tlr_fill_handoff/pending_cells.json"
)
GROUP = "td3-amo-jax-antmaze-te1-tlr-fill"
TLR_CONFIG = {
    "3e-4": ROOT / "configs" / "td3_amo_tlr3e-4.yaml",
    "1e-3": ROOT / "configs" / "td3_amo_tlr1e-3.yaml",
    "2e-3": ROOT / "configs" / "td3_amo_tlr2e-3.yaml",
}
ENV_SHORT = {
...
def run_id(environment: str, seed: int, tag: str) -> str:
    return f"td3amo_jax_te1_tb1_tlr{tag}_{ENV_SHORT[environment]}_s{seed}"
...
def main() -> int:
    ...
    print(
        f"jax antmaze fill pending={len(pending)} max_parallel={max_parallel} out={OUT}",
        flush=True,
    )
