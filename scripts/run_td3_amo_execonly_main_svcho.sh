#!/usr/bin/env bash
# TD3-AMO π_E-only main sweep (execution_only=true; α_E = L_E + L2_RMS).
# Nest: α_E 5→2→1 · seed 0→1→2→3 · env hop→walk→ant→hc · α_lr 3e-4→1e-3→2e-3
# 3×4×15×3 = 540 cells. Train all from scratch (do not reuse old L_E-only cells).
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${OUT:-$ROOT/results/td3_amo_execonly_main_seeds03}"
...
cfg_for() {
  local a="$1" alr="$2"
  echo "$ROOT/configs/td3_amo_execonly_a${a}_$(alr_tag "$alr").yaml"
}
...
QUEUE_LOG="$LOGDIR/queue_execonly_p${PARALLEL}_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$QUEUE_LOG") 2>&1
date -Is >"$OUT/EXECONLY_MAIN_LAUNCHED"
echo "=== TD3-AMO π_E-only main nest α→seed→env→α_lr PARALLEL=$PARALLEL start $(date -Is) ==="
echo "OUT=$OUT loss=L_E+L2_RMS nest=α5→2→1"
...
# Next nest-order cell that is not DONE / live. Alpha 5 fills before 2, then 1.
start_next_cell() {
...
echo "PARALLEL=$PARALLEL (nest α=5 then 2 then 1; fill next unfinished cell)"
...
echo "=== EXECONLY MAIN QUEUE COMPLETE $(date -Is) total=$total fail=$fail ==="
