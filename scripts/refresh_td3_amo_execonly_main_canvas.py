#!/usr/bin/env python3
"""Collect TD3-AMO π_E-only main 540 (α=5,2,1); rewrite Cursor canvas + PROGRESS.md."""
...
OUT = Path("/home/svcho/amo/results/td3_amo_execonly_main_seeds03")
JOBS = OUT / "jobs"
CANVAS = Path(
    "/home/svcho/.cursor/projects/home-svcho/canvases/td3-amo-execonly-main.canvas.tsx"
)
...
def collect() -> dict:
    ...
    return {
        ...
        "schedule": (
            "π_E-only · L_E+L2_RMS · α_E 5→1 then 2 · seed0–3 · "
...
def write_progress(data: dict) -> None:
    lines = [
        "# TD3-AMO π_E-only main",
        "",
        f"**Snapshot:** {data['updated_at']}",
        f"**Setting:** `execution_only=true` · α_E loss=L_E+L2_RMS · nest TD3 5→1 then IQL then both 2 · **{data['total']}** cells",
        f"**OUT:** `{OUT}`",
        "",
        "## Live",
        "",
