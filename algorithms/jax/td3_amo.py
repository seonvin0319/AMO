...
class Agent(BaseAgent):
    ...
    def bootstrap_terms(self, scale, state, inner_batch, outer_batch, actor_name="bootstrap"):
        ...
        # Returns (l1, l2_rms, l2_sq) where l2_sq is mean-square (no sqrt).
        l1, _, _ = self._bootstrap_terms(
            scale, state, inner_batch, outer_batch, actor_name
        )
        with jax.default_matmul_precision("highest"):
            _, l2_rms, l2_sq = self._bootstrap_terms(
                scale, state, inner_batch, outer_batch, actor_name
            )
        return l1, l2_rms, l2_sq

    def _bootstrap_terms(self, scale, state, inner_batch, outer_batch, actor_name="bootstrap"):
        o = self.ops
