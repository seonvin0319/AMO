...
\qquad\mathcal L_{\alpha_B}=L_{2,\mathrm{RMS},B}.
\]

`bootstrap_loss=l2_rms` is the main default. `L1_B` remains a diagnostic; it is not included in the main meta-gradient. The optional `l1_l2_rms` ablation uses `L1_B + L2_RMS_B`. The Q and BC terms remain in both actors' inner losses. Initial alpha_E=alpha_B=5 is the main configuration.
...
The historical combined-loss GPU regression matched Torch's summed bootstrap-gradient sign on all
