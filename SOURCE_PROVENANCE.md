...
- TD3+AMO defaults to the completed ext_csv locomotion seeds 1–3 profile: initial `alpha_E=alpha_B=2` (legacy `T_E=T_B=1`), `alpha_lr=0.001`, three critic hidden layers of width 256 with LayerNorm. The corresponding AntMaze profile also appears in the logs.
- `configs/td3_amo_band95.yaml` selects the first-priority values in `scripts/launch_amo_adaptive_multiscale_loco9_tlr3e4_band95_seeds1to3.py` from the uploaded TD3 branch. It uses `alpha_lr=0.0003`, environment-specific equal initial scales (legacy T ×2), evaluation every 20,000 steps and five final rounds. It is a locomotion profile, not an AntMaze selection rule.
