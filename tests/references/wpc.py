# Extracted without semantic changes from ASPC/algorithms/offline/wpc.py; see SOURCE_PROVENANCE.md.
import copy
import math
import json
from typing import *
...
class TD3_BC:
    ...
    def __init__(self, max_action: float, actor: nn.Module, actor_optimizer: torch.optim.Optimizer, critic_1: nn.Module, critic_1_optimizer: torch.optim.Optimizer, critic_2: nn.Module, critic_2_optimizer: torch.optim.Optimizer, vnet: nn.Module, vnet_optimizer: torch.optim.Optimizer, discount: float=0.99, tau: float=0.005, policy_noise: float=0.2, noise_clip: float=0.5, policy_freq: int=2, alpha: float=2.5, device: str='cpu'):
        self.actor = actor
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        log_dict['q_value'] = current_q1.mean().item()
        ...
        vnet_loss = F.mse_loss(target_q, value)
        self.vnet_optimizer.zero_grad()
        vnet_loss.backward()
        self.vnet_optimizer.step()
        bc_weight = 1.0 * (q1 > value.detach())
