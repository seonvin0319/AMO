# Extracted without semantic changes from ASPC/algorithms/offline/iql.py; see SOURCE_PROVENANCE.md.
import copy
import math
import json
from typing import *
...
class MLP(nn.Module):
    ...
    def __init__(self, dims, activation_fn: Callable[[], nn.Module]=nn.ReLU, output_activation_fn: Callable[[], nn.Module]=None, squeeze_output: bool=False, dropout: Optional[float]=None):
        super().__init__()
        n_dims = len(dims)
        if n_dims < 2:
...
class GaussianPolicy(nn.Module):
    ...
    def __init__(self, state_dim: int, act_dim: int, max_action: float, hidden_dim: int=256, n_hidden: int=2, dropout: Optional[float]=None):
        super().__init__()
...
class DeterministicPolicy(nn.Module):
    ...
    def __init__(self, state_dim: int, act_dim: int, max_action: float, hidden_dim: int=256, n_hidden: int=2, dropout: Optional[float]=None):
        super().__init__()
        self.net = MLP([state_dim, *[hidden_dim] * n_hidden, act_dim], output_activation_fn=nn.Tanh, dropout=dropout)
        self.max_action = max_action
...
class ImplicitQLearning:
    ...
    def _update_q(self, next_v: torch.Tensor, observations: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor, terminals: torch.Tensor, log_dict: Dict):
    ...
    def _update_policy(self, adv: torch.Tensor, observations: torch.Tensor, actions: torch.Tensor, log_dict: Dict):
